...
resource "yandex_alb_load_balancer" "my-balancer" {
  ...
  allocation_policy {
    location {
      ...
      subnet_id = yandex_vpc_subnet.private-1.id 
    }#Вот тут с зонами доступности тоже непоняточка небольшая, вообще как я понял, баланисровщик должен базироваться во всех зонах где есть соответствующие машины для балансировки трафика между ними. Хотя может это просто зона локализации.
    location {
